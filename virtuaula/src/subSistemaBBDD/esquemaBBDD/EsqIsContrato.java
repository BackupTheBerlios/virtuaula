...
/**
 * Clase que gestiona las tabla contrato de nuestra base de datos.
 * Implementa las operaciones básicas de inserción,consulta,eliminación y edición de tuplas 
 * de esta tabla.
 * 
 *
 */
public class EsqIsContrato extends EsquemaBBDD {

	//Declaramos el log de la clase
	private static final Logger log = Logger.getLogger(EsqIsContrato.class);
	...
	public boolean insertar(ObjetoBBDD obj) {
		...
		if (bResultadoConexion && !obj.dameValor(Constantes.ID_ISCONTRATO_ISUSUARIO_DNI).equals("")
			...
			sQuery = "INSERT INTO " + super.getBd()+ "." + Constantes.TABLA_CONTRATO + "(" + Constantes.ID_ISCONTRATO_ISUSUARIO_DNI + "," + Constantes.CONTRATO_ISNOMINA_IDISNOMINA + "," + Constantes.CONTRATO_TIPO + ") VALUES('" + obj.dameValor(Constantes.ID_ISCONTRATO_ISUSUARIO_DNI) + "',"+obj.dameValor(Constantes.CONTRATO_ISNOMINA_IDISNOMINA) + ",'" + obj.dameValor(Constantes.CONTRATO_TIPO) + "');";	
	
			try {
				numFilas = instruccion.executeUpdate(sQuery);
				bResultado = (numFilas != 0);
				log.info("Se han insertado " + numFilas + " filas en la tabla " + Constantes.TABLA_CONTRATO);
			} catch (SQLException e) {
				log.error("Error al insertar ObjetoBBDD en " + Constantes.TABLA_CONTRATO);
				log.error(e.getMessage());
	...
	public boolean borrar(ObjetoCriterio obj) {
		...
		if (obj.dameNumCampos() != 0) {
			...
			if (bResultadoConexion) {
				...
				sQuery = "DELETE FROM " + super.getBd()+ "." + Constantes.TABLA_CONTRATO + " WHERE "+obj.dameCampo()+"=" + obj.dameValor(obj.dameCampo());
				...
				try {
					...
					numFilas = instruccion.executeUpdate(sQuery);
					bResultado = (numFilas != 0);
					log.info("Se han borrado " + numFilas + " registros de la tabla " + Constantes.TABLA_CONTRATO);
				} catch (SQLException e) {
					log.error("Error al borrar ObjetoBBDD en " + Constantes.TABLA_CONTRATO);
					log.error(e.getMessage());
	...
	/**
	 * Edita los campos seleccionados de obj en la tabla contrato.
...
//		variables a utilizar
		...
		if (obj.dameNumCampos() != 0) {
			...
			if (bResultadoConexion) {
				...
				//Como mínimo tiene que haber un criterio de búsqueda.
				//Aparte el valor me lo tiene que saber dar del campo que pongo en la condición
				sQuery = "UPDATE " + super.getBd()+ "." + Constantes.TABLA_CONTRATO + 
				...
				if (obj.dameNumCampos() > 1) {
					...
					sQuery += " where "+Constantes.ID_ISCONTRATO_ISUSUARIO_DNI +"="+obj.dameValor(Constantes.ID_ISCONTRATO_ISUSUARIO_DNI) ; 
				}
				...
				try {
					...
					numFilas = instruccion.executeUpdate(sQuery);
					bResultado = (numFilas != 0);
					log.info("Se han actualizado " + numFilas + " registros de la tabla " + Constantes.TABLA_CONTRATO);
				} catch (SQLException e) {
					log.error("Error al editar ObjetoBBDD en " + Constantes.TABLA_CONTRATO);
					log.error(e.getMessage());
	...
	public EsquemaBBDD clonar() {
		EsqIsContrato esqIsContrato = new EsqIsContrato ();
		return esqIsContrato;
	}
