...
/**
 * Clase que gestiona las tabla isHorarioHasIsAula de nuestra base de datos.
 * Implementa las operaciones básicas de inserción,consulta,eliminación y edición de tuplas 
 * de esta tabla.
 * 
 *
 */
public class EsqIsHorarioHasIsAula extends EsquemaBBDD {

	//Declaramos el log de la clase
	private static final Logger log = Logger.getLogger(EsqIsHorarioHasIsAula.class);
	...
	public boolean insertar(ObjetoBBDD obj) {
		...
		if (bResultadoConexion && !obj.dameValor(Constantes.ID_HAS_ISAULA_IDISAULA).equals("")
				&& !obj.dameValor(Constantes.ID_HAS_ISHORARIO_IDISHORARIO ).equals("")
				&& !obj.dameValor(Constantes.ISHORARIO_HAS_ISAULA_ISCURSO_IDISCURSO).equals("")) {
			try {
			...
			sQuery = "INSERT INTO " + super.getBd()+ "." + Constantes.TABLA_ISHORARIO_HAS_ISAULA + "(" + Constantes.ID_HAS_ISHORARIO_IDISHORARIO + "," + Constantes.ID_HAS_ISAULA_IDISAULA + "," + Constantes.ISHORARIO_HAS_ISAULA_ISCURSO_IDISCURSO + ") VALUES(" + obj.dameValor(Constantes.ID_HAS_ISHORARIO_IDISHORARIO) + ","+obj.dameValor(Constantes.ID_HAS_ISAULA_IDISAULA) + ","+obj.dameValor(Constantes.ISHORARIO_HAS_ISAULA_ISCURSO_IDISCURSO) + ");";	
	
			try {
				numFilas = instruccion.executeUpdate(sQuery);
				bResultado = (numFilas != 0);
				log.info("Se han insertado " + numFilas + " filas en la tabla " + Constantes.TABLA_ISHORARIO_HAS_ISAULA);
			} catch (SQLException e) {
				log.error("Error al insertar ObjetoBBDD en " + Constantes.TABLA_ISHORARIO_HAS_ISAULA);
				log.error(e.getMessage());
	...
	public boolean borrar(ObjetoCriterio obj) {
		...
		if (obj.dameNumCampos() != 0) {
			...
			if (bResultadoConexion) {
				...
				sQuery = "DELETE FROM " + super.getBd()+ "." + Constantes.TABLA_ISHORARIO_HAS_ISAULA + " WHERE "+obj.dameCampo()+"=" + obj.dameValor(obj.dameCampo());
				...
				try {
					...
					numFilas = instruccion.executeUpdate(sQuery);
					bResultado = (numFilas != 0);
					log.info("Se han borrado " + numFilas + " registros de la tabla " + Constantes.TABLA_ISHORARIO_HAS_ISAULA);
				} catch (SQLException e) {
					log.error("Error al borrar ObjetoBBDD en " + Constantes.TABLA_ISHORARIO_HAS_ISAULA);
					log.error(e.getMessage());
	...
	public ListaObjetoBBDD consultar(ObjetoCriterio obj) {
		...
		if (bResultadoConexion) {
			...
			sQuery = "SELECT * FROM " + Constantes.TABLA_ISHORARIO_HAS_ISAULA;
			if (obj.dameNumCampos() > 0) {
				sQuery += " WHERE ";
			...
			try {
				...
				while (resultSet.next()) {
					ObjetoBBDD objetoBBDD = creadorObjetoBBDD.crear(creadorObjetoBBDD.IshorarioHasIsaula);
					objetoBBDD.cambiaValor(Constantes.ID_HAS_ISHORARIO_IDISHORARIO, resultSet.getString(Constantes.ID_HAS_ISHORARIO_IDISHORARIO));
					objetoBBDD.cambiaValor(Constantes.ID_HAS_ISAULA_IDISAULA, resultSet.getString(Constantes.ID_HAS_ISAULA_IDISAULA));
					objetoBBDD.cambiaValor(Constantes.ISHORARIO_HAS_ISAULA_ISCURSO_IDISCURSO, resultSet.getString(Constantes.ISHORARIO_HAS_ISAULA_ISCURSO_IDISCURSO));				
					listaObjetoBBDDAbs.insertar(posicion++, objetoBBDD);
				}								
				
			} catch (SQLException e) {
				log.error("Error al consultar ObjetoBBDD en " + Constantes.TABLA_ISHORARIO_HAS_ISAULA);
				log.error(e.getMessage());
	...
	/**
	 * Edita los campos seleccionados de obj en la tabla isHorarioHasIsAula.
...
//		variables a utilizar
		...
		if (obj.dameNumCampos() != 0) {
			...
			if (bResultadoConexion) {
				...
				//Como mínimo tiene que haber un criterio de búsqueda.
				//Aparte el valor me lo tiene que saber dar del campo que pongo en la condición
				sQuery = "UPDATE " + super.getBd()+ "." + Constantes.TABLA_ISHORARIO_HAS_ISAULA + 
				...
				if (obj.dameNumCampos() > 1) {
					...
					sQuery += " where "+Constantes.ID_HAS_ISHORARIO_IDISHORARIO +"="+obj.dameValor(Constantes.ID_HAS_ISHORARIO_IDISHORARIO) ; 
				}
				...
				try {
					...
					numFilas = instruccion.executeUpdate(sQuery);
					bResultado = (numFilas != 0);
					log.info("Se han actualizado " + numFilas + " registros de la tabla " + Constantes.TABLA_ISHORARIO_HAS_ISAULA);
				} catch (SQLException e) {
					log.error("Error al editar ObjetoBBDD en " + Constantes.TABLA_ISHORARIO_HAS_ISAULA);
					log.error(e.getMessage());
	...
	public EsquemaBBDD clonar() {
		EsqIsHorarioHasIsAula esqIsHorarioHasIsAula = new EsqIsHorarioHasIsAula ();
		return esqIsHorarioHasIsAula;
	}
