 ...
 * profesores. Se ayudara de otros gestores para realizar dichas operaciones.
 * Tambien se encarga de comprobar que los datos de un profesor esten bien
 * introducidos.
 * 
...
public class Profesorado {
	...
	/**
	 * Consulta un profesor con las caracteristicas de un bean profesor que le
	...
	/**
	 * Realizamos modificaciones sobre una ficha con las caracteristicas de
	 * beanFicha
	 * 
	 * @param beanFicha
	 * @return ListaObjetoBean
	...
	/**
	 * Nos devueve una lista de los alumnos de un curso que le pasamos por
	 * parametro
	 * 
	 * @param beanCurso
	 * @return ListaObjetoBean
	...
	/**
	 * Editar los datos de un profesor pasado por parámetro, enviandole un
	 * mensaje automatico informandole de los cambios
	 * 
	 * @param profesor
	 * @return ListaObjetoBean Contiene los errores detectados al editar
	 */
	public ListaObjetoBean editar(ObjetoBean profesor) {
		...
		usu.cambiaValor(Constantes.ID_ISUSUARIO_DNI, profesor
	...
	/**
	 * Realiza la publicacion de las notas, borrando las fichas de los alumnos y
	 * pasando el curso a inactivo
	 * 
	 * @param beanCurso
	...
	public boolean PublicarNota(ObjetoBean beanCurso) {
		...
		aviso.cambiaValor(Constantes.AVISOS_TEXTO, "Las notas del curso "
				+ beanCurso.dameValor(Constantes.CURSO_NOMBRE)
				+ " han sido publicadas");
		GestorAvisos gestAvisos = new GestorAvisos();
	...
	/**
	 * Consulta los contratos y devuelve una lista con los que se correspondan
	 * con el contrato que le pasamos
	 * 
	 * @param contrato
	 * @return ListaObjetoBean
	 */
	...
	/**
	 * Devuelve una lista de nominas que se correspondan con la que le pasamos
	 * por parametro
	 * 
	 * @param nomina
	 * @return ListaObjetoBean
	 ...
	 * 
	 * @param curso
	 *            del cual queremos hallar su expediente
	 * @return ListaObjetoBean
	 */
	...
	/**
	 * Comprueba los datos personales de un profesor
	 * 
	 * @param bean
	...
	private ListaObjetoBean comprobarDatosPersonales(ObjetoBean bean) {
		...
		// El apellido no debe ser un campo vacío
			...
			try {
				int telf = Integer.parseInt(bean
						.dameValor(Constantes.PROFESOR_TELEFONO));
				if (telf < 0) {
	...
	/**
	 * Comprueba que los datos del profesor,de su nomina y su contraro sean
	 * correctos para insertarlo
	 * 
	 * @param bean
	 * @return ListaObjetoBean Contiene los errores detectados
	 */
	private ListaObjetoBean comprobar(ObjetoBean bean, ObjetoBean nomina,
			ObjetoBean contrato) {
		CreadorListaObjetoBean c = new CreadorListaObjetoBean();
		CreadorBean cBean = new CreadorBean();
		ListaObjetoBean listaerror = c.crear();
		...
		if (!nomina.dameValor(Constantes.NOMINA_CUENTA_INGRESOS).equals("")) {
			try {
				int telf = Integer.parseInt(nomina
						.dameValor(Constantes.NOMINA_CUENTA_INGRESOS));
				if (telf < 0) {
		...
		if (!nomina.dameValor(Constantes.NOMINA_CANTIDAD).equals("")) {
			try {
				int telf = Integer.parseInt(nomina
						.dameValor(Constantes.NOMINA_CANTIDAD));
				if (telf < 0) {
	...
	/**
	 * Este metodo introduce un usuario profesor en el sistema y genera la
	 * contraseña para el usuario. Se le manda un aviso al profesor y otro a la
	 * secretaria informando de la accion. Tambien introducira la nomina y el
	 * contrato para el contable.
	 * 
	 * @param profesor
	 * @param usuario
	 * @param nomina
	 ...
	 * @return ListaObjetoBean Contiene los errores detectados
	 */
	public ListaObjetoBean contratarProfesor(ObjetoBean profesor,
			ObjetoBean usuario, ObjetoBean nomina, ObjetoBean contrato) {
		BBDDFachada bdf = BBDDFachada.getInstance();
		CreadorBean creador = new CreadorBean();
		ListaObjetoBean liserror = this.comprobar(profesor, nomina, contrato);
		if (liserror.esVacio()) {// no hay errores en los datos del profesor
			...
			if (!bdf.insertar(usuario)) {// genero un error
				...
				// solo tendremos una nomina con un numero de ingresos y un
				// sueldo
				nomina = lisnom.dameObjeto(0);
				contrato.cambiaValor(Constantes.CONTRATO_ISNOMINA_IDISNOMINA,
						nomina.dameValor(Constantes.ID_ISNOMINA));
